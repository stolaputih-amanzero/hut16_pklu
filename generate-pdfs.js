...
const generatePdfComponent = (type) => `import {
    ...
    const Header = () => (
        <View style={styles.headerWrapper} fixed>
            <View style={styles.headerLeft}>
                ...
                <View>
                    <Text style={styles.headerTitle}>{isId ? 'PROPOSAL ${type === 'donatur' ? 'DONATUR' : 'SPONSOR'}' : '${type === 'donatur' ? 'DONOR' : 'SPONSOR'} PROPOSAL'}</Text>
...
const donaturContent = generatePdfComponent('donatur')
fs.writeFileSync(path.join(process.cwd(), 'src/components/pdf/ProposalDonaturPDF.tsx'), donaturContent)

const sponsorContent = generatePdfComponent('sponsor')
fs.writeFileSync(path.join(process.cwd(), 'src/components/pdf/ProposalSponsorPDF.tsx'), sponsorContent)

console.log('Successfully generated full 9-page PDFs matching the master document perfectly!');
