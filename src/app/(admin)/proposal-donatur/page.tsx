...
export default function ProposalDonaturPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Fetch pending donatur proposals
    ...
    const handleCategoryChange = (value: string) => {
        setFormData(prev => ({ ...prev, donatur_category: value }))
    }
    ...
    const handleGenerateProposal = async () => {
        ...
        try {
            ...
            const number = await getNextNumber('donatur', 2026)
    ...
    const confirmPayment = async () => {
        ...
        try {
            ...
            setIsConfirmed(true)
            toast.success('Dana berhasil dikonfirmasi & token dibuat')
        ...
        try {
            ...
            const waLink = buildWhatsAppLink(
                ...
                formData.language as 'id' | 'en',
                {
                    ...
                    category: proposal.donatur_category,
                    token_url: tokenUrl
                }
            )
                ...
                <Card className="border-emerald shadow-emerald overflow-hidden bg-[#022c22]/40 backdrop-blur-xl">
                    <CardHeader className="border-b border-[#D4AF37]/20 pb-6 bg-[#022c22]/60">
                        <CardTitle className="text-xl font-bold text-[#FDFBF7] flex items-center">
                            ...
                            Verifikasi & Konfirmasi Pembayaran (Poka-Yoke)
                        </CardTitle>
                        <p className="text-[#FDFBF7]/60 mt-1 text-xs">
                            Cari dan pilih proposal berstatus pending dari database untuk diverifikasi secara aman.
                        </p>
                    </CardHeader>
                    <CardContent className="pt-6 space-y-6">
                        <div className="space-y-2">
                            <Label className="text-[#D4AF37] font-semibold text-sm">Cari Proposal Pending</Label>
                            <SearchableSelect
                                options={pendingProposals.map(p => ({
                                    value: p.id,
                                    label: `${p.number} - ${p.name}`,
                                    sublabel: `Rp ${p.contribution_value?.toLocaleString('id-ID')} | Jemaat: ${p.congregation || '-'}`
                                }))}
                                value={selectedVerificationId}
                                onChange={handleVerificationSelect}
                                placeholder="Masukkan nomor proposal atau nama donatur..."
                ...
                <Card className="border-emerald shadow-emerald overflow-hidden bg-[#022c22]/40 backdrop-blur-xl">
                    ...
                    <CardContent className="pt-8">
                        ...
                        <div className="space-y-6">
                            ...
                            {/* Nama Donatur */}
                            <motion.div variants={itemVariants} className="form-section bg-black/10 p-6 rounded-xl border border-[#D4AF37]/10">
                                <Label className="form-section-title">Nama Donatur</Label>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                                    <div className="space-y-2">
                                        <Label htmlFor="name" className="text-[#FDFBF7]/80">Nama Lengkap</Label>
                                        <Input
                                            id="name"
                                            name="name"
                                            value={formData.name}
                                    ...
                                    <div className="space-y-2">
                                        <Label htmlFor="display_name" className="text-[#FDFBF7]/80">Nama yang Dicantumkan</Label>
                                        <Input
                                            id="display_name"
                                            name="display_name"
                                            value={formData.display_name}
                            ...
                            {/* Bahasa */}
                            <motion.div variants={itemVariants} className="form-section bg-black/10 p-6 rounded-xl border border-[#D4AF37]/10">
                                <Label htmlFor="language" className="form-section-title">Bahasa</Label>
                                <div className="mt-4">
                                    <Select
                                        value={formData.language}
                                        onValueChange={(value) => setFormData({ ...formData, language: value })}
